...
try:
    ...
    from .model_providers import create_model_provider, test_provider_connectivity
...
class MCPWeatherAgent:
    ...
    def __init__(self, debug_logging: bool = False, prompt_type: Optional[str] = None, session_storage_dir: Optional[str] = None, mock_mode: bool = False):
        """
        ...
        Args:
            ...
            mock_mode: If True, use mock tools instead of connecting to MCP servers
        """
        ...
        if not test_provider_connectivity(self.model_provider):
            logger.warning(f"Model provider connectivity test failed for {self.model_provider.get_info()['provider']}")
        
        # MCP server configuration
        self.mock_mode = mock_mode
        self.mcp_servers = self._get_mcp_servers() if not mock_mode else {}
    ...
    def _create_mcp_clients(self) -> List[tuple[str, MCPClient]]:
        ...
        if self.mock_mode:
            # Return empty list for mock mode - tools will be created separately
            logger.info("Mock mode enabled - skipping MCP client creation")
            return []
    ...
    async def test_connectivity(self) -> Dict[str, bool]:
        ...
        if self.mock_mode:
            ...
            logger.info("Mock mode enabled - simulating successful connectivity")
            return {
                "forecast": True,
                "historical": True,
                "agricultural": True
    ...
    def _process_with_clients_sync(self, message: str, clients: List[tuple[str, MCPClient]], 
        ...
        """
        if self.mock_mode:
            # Use mock tools instead of MCP clients
            all_tools = create_mock_tools()
            logger.info(f"Using {len(all_tools)} mock tools")
            
            if self.debug_logging:
                print(f"\n🔧 Available mock tools: {len(all_tools)}")
    ...
    async def query(self, message: str, session_id: Optional[str] = None) -> str:
        ...
        if not self.mock_mode and not clients:
    ...
    def _process_structured_query_sync(self, message: str, clients: List[tuple[str, MCPClient]], 
        ...
        """
        if self.mock_mode:
            # Use mock tools for structured queries
            all_tools = create_mock_tools()
            logger.info(f"Using {len(all_tools)} mock tools for structured query")
            ...
            try:
                ...
                return fallback_response, agent.messages
            except Exception as e:
    ...
    def get_agent_info(self) -> Dict[str, Any]:
        ...
        # Get provider info
        provider_info = self.model_provider.get_info()
        ...
        return {
            ...
            "mcp_servers": list(self.mcp_servers.keys()) if not self.mock_mode else ["mock"],
...
async def create_weather_agent(debug_logging: bool = False, prompt_type: Optional[str] = None, mock_mode: bool = False) -> MCPWeatherAgent:
    """
    ...
    """
    agent = MCPWeatherAgent(debug_logging=debug_logging, prompt_type=prompt_type, mock_mode=mock_mode)
    
    # Test connectivity
    connectivity = await agent.test_connectivity()
    if not mock_mode and not any(connectivity.values()):
