...
### 🚀 From LangGraph to Strands

**LangGraph Approach**: Manual setup of tools, checkpointers, and orchestration logic.

**AWS Strands Revolution**: Native MCP integration with automatic tool discovery and simplified agent creation.
...
- 50% less code compared to LangGraph implementations
...
### Option 1: Run Weather Agent Directly with Python (Fastest)
...
```bash
# All handled by scripts - no manual setup needed!
./scripts/start.sh     # Starts everything with AWS creds
...
./scripts/setup-python.sh    # Sets up venv and installs deps
./scripts/start_servers.sh   # Starts MCP servers
python main.py              # Run main app
./scripts/stop_servers.sh   # Stop servers
```

### The Magic: AWS Credentials in Docker

Our scripts handle the complexity of AWS credentials automatically:

```bash
# scripts/start.sh does this for you:
...
|--------|---------|---------|
| `aws-setup.sh` | Configure AWS Bedrock | Creates `.env` with model config |
| `start.sh` | Start Docker services | Auto-exports AWS credentials |
| `stop.sh` | Stop all containers | Cleanup and shutdown |
| `test_docker.sh` | Test running services | Health & functionality checks |
| `test_agent.sh` | Test agent locally | Quick functionality test |
| `run_tests.sh` | Full test suite | Unit & integration tests |
| `start_servers.sh` | Start MCP servers | Local Python execution |
| `stop_servers.sh` | Stop MCP servers | Cleanup server processes |
