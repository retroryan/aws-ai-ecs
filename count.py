...
def main():
    ...
    found_in_dirs = set() # Set to store unique directories where files were counted
    ...
    for dirpath, dirnames, filenames in os.walk(START_DIR, topdown=True):
        ...
        for filename in filenames:
            ...
            if extension in TARGET_EXTENSIONS:
                ...
                found_in_dirs.add(relative_dirpath)
    ...
    print("-" * 20)
    print("Directories containing counted files:")
    if not found_in_dirs:
        print("  (None)")
    else:
        # Sort for readability and consistency
        for directory in sorted(list(found_in_dirs)):
            print(f"  - {directory}")
