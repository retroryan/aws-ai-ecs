...
# Python ECS Template - Build and Push Script
...
# to ensure compatibility with ECS Fargate. Docker build uses --platform flag
# to build for the correct architecture regardless of the local machine.
...
echo "=================================================="
echo "Build & Push Python Images to ECR"
echo "=================================================="
echo ""
...
for component in "${COMPONENTS[@]}"; do
    ...
    docker build --platform linux/amd64 -t "${IMAGE_NAME}:${VERSION_TAG}" "./${component}" > build-${component}.log 2>&1 &
    BUILD_PID=$!
